...
<footer class="w-full py-section-gap px-margin-mobile md:px-margin-desktop bg-deep-royal text-pure-white overflow-hidden">
    <div class="max-w-[1280px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-gutter">
        ...
        <div class="space-y-6 min-w-0">
            ...
            <p class="font-body-md text-surface-container-highest opacity-70">Request a project quote or product catalog.</p>
             <form class="flex flex-col sm:flex-row gap-4 min-w-0" method="POST" action="/inquiry">
                <input type="hidden" name="csrf_token" value="<?= Security::generateCsrfToken() ?>">
                <div style="position:absolute;left:-9999px" aria-hidden="true"><input type="text" name="website" tabindex="-1" autocomplete="off"></div>
                <input type="email" name="email" class="min-w-0 w-full flex-1 bg-pure-white/10 border-pure-white/20 rounded-lg px-4 py-3 text-pure-white placeholder:text-pure-white/40 focus:ring-2 focus:ring-vibrant-amber focus:border-transparent outline-none" placeholder="Email Address" required>
            ...
            <div class="pt-8 border-t border-pure-white/10">
                <span class="font-label-caps text-label-caps text-surface-container-highest opacity-50 break-words">&copy; <?= date('Y') ?> White Lotus Trading - F.Z.E. All Rights Reserved.</span>
            </div>
        </div>
    </div>
</footer>
